...
def test_max(t):
    ...
    assert out[0, 0, 0] == max([t[d, 0, 0] for d in range(k)])
    out = minitorch.max(t, 1)
    assert out[0, 0, 0] == max([t[0, d, 0] for d in range(j)])
    out = minitorch.max(t, 2)
    assert out[0, 0, 0] == max([t[0, 0, d] for d in range(i)])
    assert out[0, 0, 0] == minitorch.max(minitorch.max(minitorch.max(t, 0), 1), 2)
