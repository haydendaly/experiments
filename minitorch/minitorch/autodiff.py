...
def backpropagate(variable, deriv):
    ...
    # step 0
    topological_order = deque(topological_sort(variable))
    derivatives = dict()
    ...
    while topological_order:
        ...
        # step b
            ...
            # b.1
            backprop = node.history.backprop_step(
                derivatives.get(node.unique_id, deriv)
            )
            # b.2 and b.3
            for var, derivative in backprop:
                derivatives[var.unique_id] = (
                    derivatives.get(var.unique_id, 0) + derivative
                )
