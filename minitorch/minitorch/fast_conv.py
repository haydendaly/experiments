        ...
        for j in prange(out_channels):
            for k in prange(out_width):
                ...
                for l in prange(in_channels):
                    for m in prange(kw):
                        new_w = k + m - reverse * (kw - 1)
                        if not 0 <= new_w < width:
                            continue
                        in_pos = i * s1[0] + l * s1[1] + new_w * s1[2]
                        w_pos = j * s2[0] + l * s2[1] + m * s2[2]
                        out_sum += input[in_pos] * weight[w_pos]
                    ...
                    for w in prange(in_channels):
                        for m in prange(kh):
                            for n in prange(kw):
                                ...
                                weight_pos = (
                                    s2[0] * j + s2[1] * w + s2[2] * m + s2[3] * n
                                )
                                out_sum += (
                                    input[int(input_pos)] * weight[int(weight_pos)]
                                )
                    out_pos = i * s3[0] + j * s3[1] + k * s3[2] + l * s3[3]
                    out[out_pos] = out_sum
