...
def make_tensor_backend(tensor_ops, is_cuda=False):
    ...
    class Backend:
        ...
        class Exp(Function):
            ...
            def backward(ctx, grad_output):
                result = ctx.saved_values
                return mul_zip(result, grad_output)
        ...
        class Permute(Function):
            ...
            def forward(ctx, a, order):
                ...
                return Tensor.make(
                    data._storage, data.shape, strides=data.strides, backend=a.backend
                )
            ...
            def backward(ctx, grad_output):
                a = ctx.saved_values
                return Tensor.make(
                    grad_output._tensor._storage, a, backend=grad_output.backend
                )
