...
def index_to_position(index, strides):
    ...
    """
    position = 0
    for i_pos, s_pos in zip(index, strides):
        position += i_pos * s_pos
...
def to_index(ordinal, shape, out_index):
    ...
    """
    cur_ord = int(ordinal)
    for i_new in range(len(shape) - 1, -1, -1):
        sh = shape[i_new]
        out_index[i_new] = int(cur_ord % sh)
        cur_ord = cur_ord // sh
...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    Returns:
        None : Fills in `out_index`.
    """
    for i_broad, s_broad in enumerate(shape):
        if s_broad > 1:
            out_index[i_broad] = big_index[i_broad + (len(big_shape) - len(shape))]
        else:
            out_index[i_broad] = 0
    return None
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        for i in range(len(order)):
            f_shape.append(self.shape[order[i]])
            f_stride.append(self.strides[order[i]])

        f_shape = tuple(f_shape)
        f_stride = tuple(f_stride)

        return TensorData(storage=self._storage, shape=f_shape, strides=f_stride)
